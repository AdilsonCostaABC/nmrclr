...
function ViewPermitNotSignedoff() {
    ...
    const [permitNotSignedoffList, setpermitNotSignedoffList] = useState([]);
    ...
    useEffect(() => {
        axios.get(`/api/view-permitNotSignedoff`).then(res=>{
                
                if (res.data.status===200) {
                    
                    setpermitNotSignedoffList(res.data.permitNotSignedoff);
                }
                setloading(false);
        });
    }, []);
    ...
    var a=1;
    if (loading) {
        return <h4>Loading Permit Not Signedoff...</h4>
    }
    else
    {
        ...
        <Paper sx={{ml:7,mt:1,width:800}}>
                ...
                >
                    ...
                    >
                        ...
                        <TableBody>
                                ...
                                permitNotSignedoffList.filter(permitNotSignedoff => {
                                if (query==='')
                                {
                                    ...
                                    >
                                            ...
                                            <TableCell>
                                                <Tooltip title={"View Permit not signedoff"}>
                                                <Link style={{ textDecoration:'none' }} to={`signoff-permit/${permitNotSignedoff.id}`} >
                                                        <Button
                                                            size="small"
                                                            type="button"
                                                            fullWidth
                ...
                >
                        <TablePagination
                            ...
                            component="div"
                            count={permitNotSignedoffList.length }
                            // count={20}
        ...
        <div className="container px-4">
            <div className="card mt-4">
                <div className="card-header   bg-danger">
                    <h4 className="text-white">Permit(s) Not Signedoff
...
export default ViewPermitNotSignedoff;