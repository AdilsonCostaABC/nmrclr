...
function ViewPermitNotApproved() {
    ...
    const [permitNotApprovedList, setpermitNotApprovedList] = useState([]);
    ...
    useEffect(() => {
        axios.get(`/api/view-permitNotApproved`).then(res=>{
                
                if (res.data.status===200) {
                    
                    setpermitNotApprovedList(res.data.permitNotApproved);
                }
                setloading(false);
        });
    }, []);

    var viewPermitApproved_MATERIALTABLE="";
    var a=1;
    if (loading) {
        return <h4>Loading Permit Not Approved...</h4>
    }
    else
    {
        
        viewPermitApproved_MATERIALTABLE=
            <div>
        ...
        <Paper sx={{ml:7,mt:1,width:800}}>
                ...
                >
                    ...
                    >
                        ...
                        <TableBody>
                                ...
                                permitNotApprovedList.filter(permitNotApproved => {
                                if (query==='')
                                {
                                    return permitNotApproved;
                                } else if ( ( permitNotApproved.permit_date===query) || permitNotApproved.permit_no.toLowerCase().includes(query.toLowerCase()) || permitNotApproved.company.comp_name.toLowerCase().includes(query.toLowerCase()) || permitNotApproved.points_of_entry_exit.name.toLowerCase().includes(query.toLowerCase()) ) {
                                    return permitNotApproved;
                                }
                                }).slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((permitNotApproved, index) => (  
                                    <TableRow
                                        key={permitNotApproved.id}
                                    ...
                                    >
                                            ...
                                            <Grid container>
                                                    ...
                                                    <Grid item lg={12}>  
                                                            {/* {permitNotApproved.id} */}
                                                            
                                                            {a++}
                                                    </Grid>
                                                </Grid>
                                            </TableCell>
                                            <TableCell sx={{fontWeight:'bold',color:'#00022E'}}>{permitNotApproved.permit_no}</TableCell>
                                            <TableCell>{permitNotApproved.company.comp_name}</TableCell>
                                            <TableCell >{permitNotApproved.points_of_entry_exit.name}</TableCell>
                                            <TableCell >{permitNotApproved.permit_date}</TableCell>


                                            <TableCell>
                                            <Tooltip title={"View Permit not approved"}>

                                                <Link style={{ textDecoration:'none' }} to={`approve-permit/${permitNotApproved.id}`} >
                                                        <Button
                                                            size="small"
                                                            type="button"
                                                            fullWidth
                ...
                >
                        <TablePagination
                            ...
                            component="div"
                            count={permitNotApprovedList.length }
                            // count={20}
        ...
        <div className="container px-4">
            <div className="card mt-4">
                <div className="card-header   bg-danger">
                    <h4 className="text-white">Permit(s) Not Approved
...
export default ViewPermitNotApproved;