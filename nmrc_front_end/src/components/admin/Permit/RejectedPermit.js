...
function RejectedPermit() {
    ...
    useEffect(() => {
        axios.get(`/api/view-permitRejected`).then(res=>{
                
                if (res.data.status===200) {
                    
                    setpermitNotVerifiedList(res.data.rejected);
                }
                setloading(false);
        });
    }, []);
    ...
    var a=1;
    if (loading) {
        return <h4>Loading Permit Rejected...</h4>
    }
    else
    {
        ...
        viewPermitVerify_MATERIALTABLE=
            ...
            >
                ...
                <TextField
                    sx={{ ml: 2, flex: 1 }}
                    id="standard-search"
                    label="Search Permit rejected by No/Company/POE"
                    type="search"
                    variant="standard"
                    color="warning"
                    onChange={event => setQuery(event.target.value)}
                    placeholder="Search Permit rejected by No/Company/POE"
                />
                <IconButton  
                sx={{ p: '10px' }}
                aria-label="search"
        ...
        <div className="container px-4">
            <div className="card mt-4">
                <div className="card-header   bg-danger">
                    <h4 className="text-white">Permit(s) Rejected
