...
function AddPoints_of_entry_exit() {
    ...
    const submitPoints_of_entry_exit = (e) => {
        e.preventDefault();
        const data = {
            name: points_of_entry_exitInput.name,
            region_id: points_of_entry_exitInput.region_id,
            updated_by: localStorage.getItem('auth_name'),
        }
            console.log(data);
        axios.post(`/api/store-points_of_entry_exit`, data).then(res => {
            console.log(localStorage.getItem('email'));
            console.log('hello');
            if (res.data.status === 200) {
                console.log('api yes');
                swal({
                    title: "Adding Point of entry/exit",
                    text: res.data.message,
                    icon: "success",
                    button: "ok",
                });
                setpoints_of_entry_exitInput({...points_of_entry_exitInput,
                    name:'',
                    region_id:'',
                    error_list:[],
                });
                  
                document.getElementById('Points_of_entry_exit').reset();
            } else if (res.data.status === 400) {
                setpoints_of_entry_exitInput({ ...points_of_entry_exitInput, error_list: res.data.errors });
            }
        });
        ...
        <div className="container-fluid px-4 py-5">
            ...
            <div className="card mt-4">
                <div className="card-header bg-danger">
                    <h4 className="text-white">Add Point of Entry/Exit
                ...
                <div className="card-body">
                    <form onSubmit={submitPoints_of_entry_exit} id="Points_of_entry_exit">
...
export default AddPoints_of_entry_exit;